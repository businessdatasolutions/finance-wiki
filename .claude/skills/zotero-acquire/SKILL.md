...
- After the user has dragged the sources they want into the dedicated **`finance-wiki`** Zotero collection
...
3. A dedicated collection (default name **`finance-wiki`**) holding the items to acquire.
...
uv run --with-requirements requirements.txt python fetch_zotero.py --collection finance-wiki --dry-run

# Acquire for real:
uv run --with-requirements requirements.txt python fetch_zotero.py --collection finance-wiki
```
...
| --- | --- | --- |
| `-c`, `--collection` | `finance-wiki` | Zotero collection name (case-insensitive) |
...
1. User drags sources into the `finance-wiki` Zotero collection.
2. Run `--dry-run`, review routing, then run for real. This is **Acquire** — log it as
   `acquire | zotero-batch finance-wiki (<n> items)` only if you stop here (per CLAUDE.md §Acquire).
...
notes: |
  Acquired from Zotero collection `finance-wiki`. itemType ... routed to raw/<type>/ — confirm at Process.
---
...
- **`collection not found: 'finance-wiki'`** — name typo, or the collection doesn't exist yet. The error
...
copy-the-binary path, which a self-contained script delivers..
